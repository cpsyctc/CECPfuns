---
title: "Introduction to the CECPfuns package"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introduction to the CECPfuns package}
...
This should evolve into a repository of all the functions that I (CE) have created. The first post about the package is [here](https://www.psyctc.org/Rblog/posts/2021-02-10-making-my-first-usable-package/).

The primary goal driving the creation of CECPfuns was fairly selfish: to make sure I have a single repository of my functions that I could keep up to date and where I could easily find the latest version of any potentially reusable functions I have written. However, two of my colleagues, Dr. Clara Paz and Emily Blackshaw have encouraged me to think that this might become something of use for others and I am now very much working toward that aim and, with that in mind I have put the package on GitHub at https://github.com/cpsyctc/CECPfuns.  You should always be able to install the latest version of the package with
...
I have tried to write tests for all functions that get run every time I update the package at all.  These (laboriously!) test all the sanity checking of inputs to make sure the error traps work, test all the warnings (I think I have tested all of them) and generally test the output.  However, at the moment I am not testing the ggplot output from `plotFunction()` functions as I suspect that would then involve me having to reset the test targets whenever `ggplot()` is updated.  Those tests are run on my local machine but also on the major operating systems and major R releases back to 3.5 (see above for requirements).
...
* You can [find out more about Clara here](https://investigacion.udla.edu.ec/udla_teams/clara-paz/).
* You can [find out more about Emily here](https://www.researchgate.net/profile/Emily_Blackshaw2) and about her PhD, which I am supervising, at her [ResearchGate project page](https://www.researchgate.net/project/Young-Persons-Clinical-Outcomes-in-Routine-Evaluation-YP-CORE-Scale-psychometric-properties-and-utility).
* My own sites are:
  + my biggest research programme, which overlaps with Emily's work, the CORE system at https://www.coresystemtrust.org.uk/
  + a [web site about my non-CORE work](https://www.psyctc.org/psyctc/), which I am slowly (re)building from a much earlier one
  + my ["R blog" or "SAFAQ"](https://www.psyctc.org/Rblog/) as I have called it  of pages created using R and the [distill package](https://rstudio.github.io/distill/)
  + pages which relate to a book, co-authored with Jo-anne Carlyle, coming out in autumn 2021: watch this space!
  + my [personal site](https://www.psyctc.org/pelerinage2016).

## To do list!

Sadly, this is definitely incomplete and I am sure it will continue to develop, the package too I hope, for years!
